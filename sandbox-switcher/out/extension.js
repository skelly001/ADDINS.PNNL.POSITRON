...
const addDataRoot_1 = require("./commands/addDataRoot");
...
function activate(context) {
    ...
    // Don't automatically modify workspace folders during activation
    // This can interfere with R session initialization in Positron
    // Users can manually add workspace roots using the commands if needed
    ui.log('Sandbox Switcher activated successfully');
    ui.log('Note: Workspace folders are NOT automatically modified to avoid R session issues');
    ui.log('Use "Sandbox: Switch Working Directory to Opposite Sandbox" to work with paired folders');
}
function registerCommands(context) {
    ...
    const addDataCmd = vscode.commands.registerCommand('sandbox.addDataRoot', async () => {
        ...
        await (0, addDataRoot_1.addDataRoot)(sandboxConfig, ui);
    });
...
function addWorkspaceRoots(config) {
    ...
    if (scriptsOp && dataOp) {
        ...
        if (scriptsIndex >= 0 && dataIndex >= 0) {
            ...
            vscode.workspace.updateWorkspaceFolders(scriptsOp.start, scriptsOp.deleteCount, scriptsOp.folder, dataOp.folder);
        }
        else if (scriptsIndex < 0 && dataIndex >= 0) {
            ...
            vscode.workspace.updateWorkspaceFolders(scriptsOp.start, 0, scriptsOp.folder, dataOp.folder);
        }
    }
    else if (scriptsOp) {
        // Only Scripts operation
        ui.log(`Only updating Scripts at ${scriptsOp.start}`);
        vscode.workspace.updateWorkspaceFolders(scriptsOp.start, scriptsOp.deleteCount, scriptsOp.folder);
    }
    else if (dataOp) {
        // Only Data operation
        ui.log(`Only updating Data at ${dataOp.start}`);
        vscode.workspace.updateWorkspaceFolders(dataOp.start, dataOp.deleteCount, dataOp.folder);
    }
